...
#include "controller/dartlauncher/_debug/image_viewer/image_viewer.hpp"
#include "controller/dartlauncher/_debug/image_viewer/profile.hpp"
#include <sensor_msgs/msg/detail/image__struct.hpp>

using namespace rmcs_auto_aim::util;

class CVBridgeViewer final : public rmcs_auto_aim::util::ImageViewer::ImageViewer_ {
public:
    ...
    void draw(const IAutoAimDrawable& drawable, const cv::Scalar& color) final { drawable.draw(image, color); };

    void load_image(const cv::Mat&) final {};

    void show_image() final {
        auto output_msg =
            cv_bridge::CvImage(std_msgs::msg::Header(), sensor_msgs::image_encodings::BGR8, image).toImageMsg();
    };
...
public:
    explicit RtspViewer(const std::string& name)
        ...
        swsCtx_ = sws_getContext(
            width, height, AV_PIX_FMT_BGR24, width, height, codecCtx_->pix_fmt, SWS_BILINEAR, nullptr, nullptr,
            nullptr);

        timerThread_ = std::thread([this]() { executeFunctionAtFrequency(std::chrono::milliseconds(1000 / fps)); });
    }

    void draw(const IAutoAimDrawable& drawable, const cv::Scalar& color) final { drawable.draw(image_, color); };
    ...
    void show_image() final {
        ...
        int ret                    = 0;
        ret = sws_scale(swsCtx_, srcSlice, srcStride, 0, frame_->height, frame_->data, frame_->linesize);
...
private:
    ...
    void createOutputContext(const char* url, int width, int height, int fps) {
        ...
        if (ret < 0) {
            RCLCPP_INFO(rclcpp::get_logger("RTSP Viewer"), "Error return code from avformat_write_header: %x", ret);
            return;
        }

        if (!oc) {
            RCLCPP_INFO(rclcpp::get_logger("RTSP Viewer"), "Error: Could not create output context");
        } else
            oc_ = oc;
    }
    const std::string name_;
...
std::unique_ptr<rmcs_auto_aim::util::ImageViewer::ImageViewer_> rmcs_auto_aim::util::ImageViewer::viewer_;