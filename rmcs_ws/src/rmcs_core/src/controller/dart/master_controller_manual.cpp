...
public:
    MasterControllerManual()
        ...
        register_output(
            "/dart/master_control/friction_control_velocity", output_dart_launch_velocity_, nan);
        register_output("/dart/master_control/angle_command", output_angle_control_enable_, false);
        register_output(
            "/dart/master_control/angle_control_vector", output_angle_control_,
            Eigen::Vector2d::Zero());

        register_output("/dart/master_control/filling_command", output_dart_filling_enable_, false);
        register_output(
            "/dart/master_control/vision_guide_command", output_dart_vision_guide_enbale_, false);
    }
...
private:
    ...
    void update_output_control_values() {
        ...
        output_angle_control_->x() =
            std::max(-limit_velocity, std::min(limit_velocity, yaw_control_input_));
        output_angle_control_->y() =
            std::max(-limit_velocity, std::min(limit_velocity, pitch_control_input_));
...
PLUGINLIB_EXPORT_CLASS(
    rmcs_core::controller::dartlauncher::MasterControllerManual, rmcs_executor::Component)
