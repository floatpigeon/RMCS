...
template <hardware::device::DjiMotor::Type>
struct MotorPowerParamFactory {
    constexpr static std::tuple<double, double, double> param //
        = {0, 0, 0};
};
template <>
struct MotorPowerParamFactory<hardware::device::DjiMotor::Type::M3508> {
    MotorPowerParamFactory() {}
    constexpr static std::tuple<double, double, double> param //
        = {2.958580e+00, 3.082190e-03, 0.68};
};
template <>
struct MotorPowerParamFactory<hardware::device::DjiMotor::Type::GM6020> {
    MotorPowerParamFactory() {}
    constexpr static std::tuple<double, double, double> param //
        = {2.958580e+00, 3.082190e-03, 0.68};
};
...
class MotorBase {
public:
    MotorBase(hardware::device::DjiMotor::Type motor)
...
private:
    const std::tuple<double, double, double>& create_motor(hardware::device::DjiMotor::Type motor) {
        switch (motor) {
        case hardware::device::DjiMotor::Type::M3508:
            return MotorPowerParamFactory<hardware::device::DjiMotor::Type::M3508>::param;
        case hardware::device::DjiMotor::Type::GM6020:
            return MotorPowerParamFactory<hardware::device::DjiMotor::Type::GM6020>::param;
        default: throw "Error motor type at MotorPowerParamFactory";
        }
    }
...
public:
    SteerMotor(DerivedFromBoth auto* component, const std::string& name, hardware::device::DjiMotor::Type motor)
        : MotorBase(motor)
        ...
        component->register_output(name + "/control_torque", control_torque_, std::numeric_limits<double>::quiet_NaN());
    ...
    void set_target_angle_error(double error) {
        *control_torque_ = steer_velocity_pid_calculator_.update(steer_angle_pid_calculator_.update(error));
    }
...
public:
    WheelMotor(DerivedFromBoth auto* component, const std::string& name, hardware::device::DjiMotor::Type motor)
        : MotorBase(motor)
