...
public:
    ChassisMotorCalculator(DerivedFromBoth auto* chassis)
        ...
        steers_.push_back(std::make_shared<SteerMotor>(
            chassis, "/chassis/steer/left_front", hardware::device::DjiMotor::Type::GM6020));
        steers_.push_back(std::make_shared<SteerMotor>(
            chassis, "/chassis/steer/left_back", hardware::device::DjiMotor::Type::GM6020));
        steers_.push_back(std::make_shared<SteerMotor>(
            chassis, "/chassis/steer/right_back", hardware::device::DjiMotor::Type::GM6020));
        steers_.push_back(std::make_shared<SteerMotor>(
            chassis, "/chassis/steer/right_front", hardware::device::DjiMotor::Type::GM6020));

        wheels_.push_back(std::make_shared<WheelMotor>(
            chassis, "/chassis/wheel/left_front", hardware::device::DjiMotor::Type::M3508));
        wheels_.push_back(
            std::make_shared<WheelMotor>(chassis, "/chassis/wheel/left_back", hardware::device::DjiMotor::Type::M3508));
        wheels_.push_back(std::make_shared<WheelMotor>(
            chassis, "/chassis/wheel/right_back", hardware::device::DjiMotor::Type::M3508));
        wheels_.push_back(std::make_shared<WheelMotor>(
            chassis, "/chassis/wheel/right_front", hardware::device::DjiMotor::Type::M3508));
    }
    ...
    bool calculate_wheel_velocity_and_angle(const Eigen::Vector2d& move, double spin_speed) {
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now() - last_time_point_)
                .count()
            >= 0.5) {
        ...
        double err[4] = {
            -atan2(lb_vel_angle.y(), lb_vel_angle.x()) - steers_[0]->get_current_angle(),
            -atan2(lf_vel_angle.y(), lf_vel_angle.x()) - steers_[1]->get_current_angle(),
            -atan2(rb_vel_angle.y(), rb_vel_angle.x()) - steers_[2]->get_current_angle(),
            -atan2(rf_vel_angle.y(), rf_vel_angle.x()) - steers_[3]->get_current_angle()};
        ...
        wheels_[0]->set_target_velocity(lf_vel.norm() * wheel_speed_limit * check_error_angle(err[0]));
        wheels_[1]->set_target_velocity(lb_vel.norm() * wheel_speed_limit * check_error_angle(err[1]));
        wheels_[2]->set_target_velocity(rb_vel.norm() * wheel_speed_limit * check_error_angle(err[2]));
        wheels_[3]->set_target_velocity(rf_vel.norm() * wheel_speed_limit * check_error_angle(err[3]));
...
private:
    ...
    std::chrono::time_point<std::chrono::high_resolution_clock> last_time_point_ = std::chrono::system_clock::now();
